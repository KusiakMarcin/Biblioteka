...
bool database::initDatabase(){
    ...
    if(rc){
     ...
     rc = sqlite3_prepare_v2(Db, sqlCreateTables, -1, &stmt, NULL);
        if (rc != SQLITE_OK) {
        ...
        if (rc != SQLITE_DONE) {
            qDebug() << "Failed to execute statement:\n" <<sqlite3_errmsg(this->Db);
...
bool database::addNewRental(int ClientID, int BookID, const QDate& dataWydania, const QDate& dataZwrotu) {
    sqlite3_stmt* stmt;
    const char* sql = "INSERT INTO Wypożyczenia (id, klienci_id, ksiazki_id, data_wydania, termin_oddania) VALUES (NULL, ?, ?, date('now'), date('now', '+90 days'))";
    int rc = sqlite3_prepare_v2(Db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "Failed to prepare statement: %s\n", sqlite3_errmsg(Db));
        return false;
    ...
    if (!sqlite3_step(stmt)) {
        qDebug() << "Błąd podczas dodawania wypożyczenia: " << sqlite3_errmsg(this->Db);
        sqlite3_finalize(stmt);
        return false;
    }
    sqlite3_finalize(stmt);
